  ...
  <el-dialog title="新增员工" :visible="showDialog" :before-close="handleClose">
    ...
    <el-form label-width="120px" :model="formData" :rules="rules">
      ...
      <el-form-item label="部门" prop="departmentName">
        <el-input v-model="formData.departmentName" style="width:80%" placeholder="请选择部门" @focus="getDept" />
        <el-tree v-if="showTree" v-loading="loading" :data="depts" :props="defaultName" :default-expand-all="true" @node-click="handleClick" />  <!----获取部门结构------>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        formOfEmployment: [
          { required: true, message: '请填写聘用形式', trigger: 'blur' }
        ],
        workNumber: [
  ...
  methods: {
    ...
    async getDept() {
      ...
      this.depts = changeArrToTree(depts, '')
      this.showTree = true
      this.loading = false
    },
    handleClick(node) {
